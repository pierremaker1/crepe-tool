...
import { Prospect, ProspectInsert, ProspectUpdate, ProspectStatus, WebsiteStatus, ALL_STATUSES, STATUS_LABELS, WEBSITE_STATUS_LABELS, STATUS_COLORS } from '@/lib/types'
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <Section title="Informations">
        <Field label="Nom de l'établissement *">
          <input
            ...
            placeholder="La Crêperie Bretonne"
          />
        </Field>
        <Field label="Gérant (prénom et nom)">
          <input
            className={inputCls}
            value={form.owner}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Nb avis">
            <input
              ...
              onChange={e => set('google_reviews', e.target.value)}
              type="number"
              min="0"
              placeholder="120"
            />
      ...
      <Section title="Site web">
        <div className="grid grid-cols-2 gap-2">
          {(['none','bad','correct','good'] as WebsiteStatus[]).map(ws => (
            <button
              key={ws}
              type="button"
              onClick={() => set('website_status', ws)}
              className={`py-2.5 rounded-xl text-sm font-medium border-2 transition-colors ${
                form.website_status === ws
                  ? 'border-gray-900 bg-gray-900 text-white'
                  : 'border-gray-200 bg-white text-gray-600'
              }`}
            >
              {WEBSITE_STATUS_LABELS[ws]}
            </button>
          ))}
        </div>
      </Section>
      ...
      <Section title="Visite">
        ...
        <div className="flex justify-end">
          <button
            type="button"
            onClick={() => set('visit_date', todayISO())}
        ...
        <Field label="Argument de vente">
          <textarea
            className={`${inputCls} min-h-[80px] resize-none`}
            value={form.pitch_argument}
            onChange={e => set('pitch_argument', e.target.value)}
            placeholder="Principal argument pour ce prospect..."
          />
        </Field>
      </Section>
      ...
      >
        {saving ? 'Enregistrement...' : isNew ? 'Ajouter le prospect' : 'Enregistrer les modifications'}
      </button>
      ...
      {onDelete && (
        ...
        >
          {deleting ? 'Suppression...' : confirmDelete ? 'Confirmer la suppression' : 'Supprimer ce prospect'}
        </button>
      )}
